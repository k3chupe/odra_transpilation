...
rotations are ignored). See `native_cz_cost()` in `src/odra_router/contract.py`.
...
## Budget sweep (`odra-router-bench-sweat`)
...
- Instances: `hard_Nr` (deterministic cycles over the six non-edge pairs,
  [`generator.hard_circuit`](../src/odra_router/generator.py)), dense random
...
routed circuit (see `src/odra_router/fidelity.py` and `docs/contract.md`). The
benchmark scores every solver and the Qiskit preset on this objective:

```bash
odra-router-bench-fidelity
...
generated by [`src/odra_router/generator.py`](../src/odra_router/generator.py). Useful for
...
[`src/odra_router/queko.py`](../src/odra_router/queko.py) (adapted from
...
[`src/odra_router/datasets.py`](../src/odra_router/datasets.py) loads OpenQASM 2.0 files
so external benchmarks can be dropped in without adding dependencies:

```python
from odra_router.datasets import load_qasm, qasm_circuits
...
```python
from odra_router.datasets import fetch_corpus
...
- [MQT Bench](https://pypi.org/project/mqt.bench/) — realistic algorithm
  ...
  exported QASM files through `odra_router.datasets` if needed.
